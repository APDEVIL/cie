...
// src/app/cie-written/[id]/page.tsx
...
export default function CieWrittenDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router  = useRouter();
  const recordId = Number(id);

  const { data, isLoading } = api.cieWritten.getById.useQuery({ id: recordId });

  const deleteMutation = api.cieWritten.delete.useMutation({
    onSuccess: () => {
      toast.success("Record deleted");
      router.push("/cie-written");
    },
  ...
  if (!data) {
    return (
      <div className="space-y-4">
        <PageHeader title="Record Not Found" backHref="/cie-written" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <Link href={`/cie-written/${recordId}/edit`}>
        ...
        <DeleteDialog
          label={`${data.student?.name}'s ${data.testType} record`}
